...
export class MailService {
  ...
  async sendVerificationEmail(email: string, token: string): Promise<void> {
    ...
    const text = `Welcome to WDS Shoe Store!
...
---
This email was sent by WDS Shoe Store
© ${currentYear} WDS Shoe Store. All rights reserved.`;

    await this.mailer.sendMail({
      to: email,
      subject: 'WDS Shoe Store - Verify your email',
      text,
      html,
    });
  }
...
© ${currentYear} WDS Shoe Store. All rights reserved.`;
    ...
    await this.mailer.sendMail({
      to: email,
      subject: 'WDS Shoe Store - Reset Your Password',
      text,
      html,
    });
  }
