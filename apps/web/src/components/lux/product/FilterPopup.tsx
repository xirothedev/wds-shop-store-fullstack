...
const FilterPopup = ({ isOpen ,setOpen}: FilterPopupProps) => {
  ...
  const content = (
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="relative w-full max-w-4xl rounded-2xl border border-amber-500/20 bg-gradient-to-br from-black/95 to-black/85 backdrop-blur-xl shadow-2xl">
          {/* Header */}
          ...
          <div className="flex border-b border-amber-500/10">
            {filterCategories.map((category) => (
              <div key={category.id} className="flex-1 px-6 py-4 text-center">
                <h3 className="font-semibold text-amber-400">{category.label}</h3>
              </div>
            ))}
          </div>
          ...
          {/* Content */}
          <div className="p-8">
            <div className="grid gap-6" style={{ gridTemplateColumns: `repeat(${filterCategories.length}, 1fr)` }}>
              {filterCategories.map((category) => (
